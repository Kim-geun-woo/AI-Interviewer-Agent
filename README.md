...
| 역할 | 담당자 | 주요 업무 |
| :--- | :--- | :--- |
| **사전 준비 고도화/gradio 연결** | **(본인)** | - 이력서/채용공고 분석 및 핵심 키워드 추출 - 3가지 평가 분야 기반 면접 전략 및 예시 질문 생성 - Gradio 웹 연결|
| PM | 팀원 A | -과제 총괄 진행 -State, Grpah 담당 - 결과 보고서 |
| 답변평가 고도화 | 팀원 B | -Reflection 노드 추가 및 그래프 수정, Conversation 관리 |
| 인터퓨 진행 고도화 | 팀원 C | - 조건 설계 - 조건에 따른 노드 연결(그래프 수정) |
| 질문 생성 고도화 | 팀원 D, 팀원 E | - Vector DB 구성 - 질문 생성시 참조 |
| 피드백보고서 고도화 | 팀원 F | - 보고서 틀 구성 - 보고서 내용 작성을 위한 llm 사용 구성 |


> 🔹 본 프로젝트는 팀원 전체의 긴밀한 협업으로 진행되었습니다. 특히 LangGraph의 복잡한 상태(State)를 일관되게 관리하기 위해, Excel 시트를 활용하여 각 노드의 입출력 상태를 실시간으로 공유하고 검증하며 개발을 진행했습니다. (Source 141-142)
