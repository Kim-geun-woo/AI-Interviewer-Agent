...
본 프로젝트는 **LangGraph**를 활용하여 스스로 평가를 되돌아보고(Reflection) 다음 행동을 결정하는 **동적 AI 면접관 에이전트**를 개발한 사례입니다.
...
* **참여 인원**: 총 7명 (팀 프로젝트)
* **나의 역할**: **사전 준비(Pre-processing) 고도화** 및 **Gradio UI 연동** 담당
...
| 역할 | 담당자 | 주요 업무 |
| :--- | :--- | :--- |
| **사전 준비 고도화/gradio 연결** | **(본인)** | - 이력서/채용공고 교차 분석: analyze_resume 함수의 LLM 프롬프트를 고도화하여, 이력서와 채용공고(JD)를 동시에 분석하고 양측의 핵심 키워드를 추출하도록 개선 
- 맞춤형 면접 전략 수립: generate_question_strategy 함수의 LLM 프롬프트를 수정하여, 추출된 키워드를 바탕으로 3가지 평가 분야(경험, 소통, 문제해결력) 기반의 맞춤형 면접 전략과 예시 질문을 생성하도록 개선
- Gradio 웹 인터페이스 구현: app.py 파일을 작성하여, 사용자가 이력서와 JD 파일을 업로드하고, LangGraph 에이전트와 실시간 채팅으로 면접을 진행하며, 최종 보고서를 받을 수 있는 웹 UI를 구현 및 연동|
| PM | 팀원 A | -과제 총괄 진행 -State, Grpah 담당 - 결과 보고서 |
| 답변평가 고도화 | 팀원 B | -Reflection 노드 추가 및 그래프 수정, Conversation 관리 |
| 인터퓨 진행 고도화 | 팀원 C, 팀원 D | - 조건 설계 - 조건에 따른 노드 연결(그래프 수정) |
| 질문 생성 고도화 | 팀원 E | - Vector DB 구성 - 질문 생성시 참조 |
| 피드백보고서 고도화 | 팀원 F | - 보고서 틀 구성 - 보고서 내용 작성을 위한 llm 사용 구성 |
...
| 단계 | 내용 | 주요 역할 (본인) |
| :--- | :--- | :--- |
| **v1.0 분석** | 기존 규칙 기반(if-else) 면접 Agent의 한계점 분석 (Source 90) | - |
| **v2.0 설계** | **LangGraph** 기반의 유연하고 자가-성찰(`reflect`)이 가능한 에이전트 구조 설계 | - |
| **사전 준비 (본인)** | `analyze_resume` (이력서/JD 분석), `generate_question_strategy` (면접 전략 수립) 기능 고도화 (Source 72-75) | ✅ |
| **Agent 로직 구현** | `evaluate` → `reflect` → `re_evaluate_answer` / `decide_next_step` → `generate` 등 핵심 노드 개발 (Source 76-99) | - |
